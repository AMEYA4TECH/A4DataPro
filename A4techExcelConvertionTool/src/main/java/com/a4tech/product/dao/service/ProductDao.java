...
public class ProductDao {
	...
	public void save(List<ErrorMessage> errors, String productNo,
			Integer asiNumber) {
		...
		try {
			...
			File fout = new File("D:\\A4 ESPUpdate\\ErrorFiles\\" + errorComp
					+ ".txt");
			 ...
			 * ; Query query = session.createQuery(hql); List results =
			 * query.list();
			 */
		} catch (Exception ex) {
	...
	public void save(ExternalAPIResponse errors, String productNo,
			Integer asiNumber) {
		_LOGGER.info("Enter the DAO class");
		Set<ErrorEntity> listErrorEntity = new HashSet<ErrorEntity>();
		org.hibernate.classic.Session session = null;
